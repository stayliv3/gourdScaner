...
if($_GET['type']=='sqlmap_update'){
	...
	$data=mysql_query("select * from sqlmap where status not in ('finished','Inject')");
	while($row=mysql_fetch_array($data)) {
		$key = $row['key'];
		...
		mysql_query("update sqlmap set status='{$stat}' where `key`='{$key}'");

	}
	$data=mysql_query("select * from sqlmap where status  in ('terminated')");
	while($row=mysql_fetch_array($data)){
		$key=$row['key'];
		$stat=file_get_contents("{$row['apiserver']}/scan/{$key}/data");
		$result=json_decode($stat);
		$stat=$result->data;
		if (count($stat) == 0){
			$stat = 'finished';
		}
...
//		$log=file_get_contents("{$row['apiserver']}/scan/{$key}/data");
		$log=json_encode($result->data[0]->value);

		$log_array=addslashes($log);
...
$row=mysql_query("select DISTINCT ip as api,(select count(*) from sqlmap where apiserver=api and  status not in ('finished','Inject') ) as count from apiconfig order by count limit 1");
